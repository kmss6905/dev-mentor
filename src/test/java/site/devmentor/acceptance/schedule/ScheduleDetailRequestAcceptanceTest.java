...
public class ScheduleDetailRequestAcceptanceTest extends AcceptanceTest {
    ...
    void 스케줄세부정보_수정_성공() throws Exception {
        ...
        // when
        mockMvc.perform(patch("/api/mentor/schedules/details/{detailId}", 1L)
                .content(body)
                .contentType(MediaType.APPLICATION_JSON))
            .andDo(print())
            .andExpect(status().isOk())
    ...
    @Test
    @WithMockUser("1")
    void 스케줄세부정보_삭제() throws Exception {
        // given
        saveScheduleDetail();

        // when, then
        mockMvc.perform(delete("/api/mentor/schedules/details/{detailId}", 1L)
                .contentType(MediaType.APPLICATION_JSON))
    ...
    @Test
    @WithMockUser("2")
    void 권한없는_유저_스케줄세부정보_삭제_실패() throws Exception {
        // given
        saveScheduleDetail();
